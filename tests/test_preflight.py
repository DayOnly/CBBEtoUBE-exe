...
def test_no_modlist_fails_fast():
    ...
    how many threads and how large a page file they have. The build row
    (#releases-link) needs no modlist either and sits beside it.
    ...
    assert {c.id for c in checks} == {"memory", "build", "modlist"}, (
...
def test_check_setup_names_the_build_it_is_checking():
    """#releases-link: Check setup, `check-setup` and the diagnostics zip render
    these rows, so each says which build the other rows describe -- with or
    without a modlist -- and Help's Releases link has something to compare."""
    from src import build_info
    checks = pf.run_checks(_Lay(None))
    row = {c.id: c for c in checks}["build"]
    assert row.status == pf.OK and row.detail == build_info.stamp_line(), row
    assert f"[{pf.OK}] This build: {build_info.stamp_line()}" in pf.format_checks(checks)
