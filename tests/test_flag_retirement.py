...
def test_the_census_sees_every_switch_and_would_name_a_raw_one(monkeypatch, tmp_path):
    ...
    was the `_flag` surface, not the whole one, until every such read moved to
    the helper (2026-09-16, #env-through-helpers).

    Two halves, both required: the census names NOTHING now, and it WOULD name
    a raw read planted in a tree of its own -- "nothing" from a pattern that
    matches nothing is blindness, not cleanliness. The report block that
    prints the NOT COUNTED list stays: it is the human-visible half of this.
    """
    assert fr.raw_env_kill_switches() == [], (
        "a kill switch is read straight off os.environ again; the census "
        "cannot score it -- read it through envflags.flag")
    printed = []
    rs, live = fr.rows()
    fr.report(rs, live, out=printed.append)
    assert "NOT COUNTED" not in "\n".join(printed), (
        "the census prints a NOT COUNTED block with nothing left uncounted")
    src = tmp_path / "src"
    src.mkdir()
    (src / "planted.py").write_bytes(
        b'import os\n'
        b'A = os.environ.get("CBBE2UBE_NO_PLANTED_A") != "1"\n'
        b'B = os.environ["CBBE2UBE_NO_PLANTED_B"] == "1"\n'
        b'os.environ["CBBE2UBE_NO_PAUSE"] = "1"   # a WRITE is not a read\n')
    (src / "envflags.py").write_bytes(
        b'import os\nX = os.environ.get("CBBE2UBE_NO_IN_THE_HELPER")\n')
    monkeypatch.setattr(fr, "_REPO", tmp_path)
    assert fr.raw_env_kill_switches() == [
        ("planted", "CBBE2UBE_NO_PLANTED_A"), ("planted", "CBBE2UBE_NO_PLANTED_B")]
...
def test_every_standing_switch_carries_a_verdict_status():
    """The mechanical half of E1 leaves some switches standing (default ON,
    nothing names them, the recipe does not touch them). Each one is either
    waiting on a named record or it is decoration -- and the census must say
    which, not print a bare list a reader has to research. fr.AWAITING_VERDICT
    and the standing set must agree BOTH ways: a standing switch missing from
    the table is UNRECORDED (the state the back-residual verbosity switch was
    in until 2026-09-16, when it was retired; its variable is not named here
    because the census would count that as a reference), and a table entry
    that is no longer standing is stale. #kill-switch-verdicts"""
    rs, _live = fr.rows()
    standing = {r["name"] for r in fr.standing(rs)}
    assert len(standing) >= 3, (
        f"only {len(standing)} standing switch(es) -- the binding or reference "
        f"scan drifted and this check would be vacuous")
    unrecorded = sorted(standing - set(fr.AWAITING_VERDICT))
    stale = sorted(set(fr.AWAITING_VERDICT) - standing)
    assert not unrecorded and not stale, (
        f"unrecorded standing switch(es) {unrecorded} (retire it with the "
        f"disproven-toggles recipe, or say what it waits on); stale table "
        f"entries {stale}")


def test_the_report_says_beside_each_standing_switch_what_it_waits_on():
    rs, live = fr.rows()
    printed = []
    fr.report(rs, live, out=printed.append)
    text = "\n".join(printed)
    for r in fr.standing(rs):
        i = text.index(r["name"])
        tail = text[i:].split("\n", 2)[1]
        assert "awaiting verdict:" in tail, (r["name"], tail)
    assert "STALE VERDICT ENTRY" not in text
    assert "UNRECORDED" not in text


def test_an_unrecorded_standing_switch_is_named_as_such():
    """Control: a standing switch the table does not know must be printed
    as UNRECORDED, and a table entry nothing stands behind as STALE."""
    rs, live = fr.rows()
    fake = dict(rs[0])
    fake.update(name="PLANTED_SWITCH", env="CBBE2UBE_NO_PLANTED", resolves_on=True,
                refs={}, n_refs=0, in_live_settings=False, gui=False)
    printed = []
    fr.report(rs + [fake], live, out=printed.append)
    text = "\n".join(printed)
    i = text.index("PLANTED_SWITCH")
    assert "UNRECORDED" in text[i:].split("\n", 2)[1]
    assert fr.verdict_status("NO_SUCH_SWITCH").startswith("UNRECORDED")
    assert fr.verdict_status(next(iter(fr.AWAITING_VERDICT))).startswith("awaiting verdict:")
