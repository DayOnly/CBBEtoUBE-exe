...
The `_1` variant owns it, matching the `_1` reference body the deltas are
measured against. These pin the rule and, above all, the exception: a `_0` with
no partner must still get a TRI, or single-variant armour silently loses body
morphs -- a worse bug than the one being fixed.
"""
...
def test_full_weight_variant_owns_the_tri(tmp_path):
    _mk(tmp_path, "boots_0.nif", "boots_1.nif")
    assert nc._tri_is_owning_variant(tmp_path / "boots_1.nif") is True
    assert nc._tri_is_owning_variant(tmp_path / "boots_0.nif") is False


def test_zero_weight_with_no_partner_still_generates(tmp_path):
    """The exception that matters: yielding here would ship no TRI at all."""
    _mk(tmp_path, "solo_0.nif")
    assert nc._tri_is_owning_variant(tmp_path / "solo_0.nif") is True
