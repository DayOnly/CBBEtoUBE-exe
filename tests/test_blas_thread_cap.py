...
def test_any_import_of_the_package_caps_before_numpy():
    """B-7. The two product entry points cap; a script, census or test that did
    `from src import nif_convert` did not, and paid the whole arena. The package
    __init__ runs before every src.* module, so it caps for all of them -- and
    it must not import numpy itself, or the cap would come too late."""
    out = _run("""
        import os, sys
        import src
        print("numpy" in sys.modules, os.environ.get("OPENBLAS_NUM_THREADS", "UNSET"))
    """)
    assert out == "False 1", out
...
@pytest.mark.skipif(os.name != "nt", reason="commit charge is a Windows notion")
@pytest.mark.skipif((os.cpu_count() or 1) < 2, reason="no arena to save")
def test_a_direct_nif_convert_import_pays_no_arena():
    """B-7, in commit charge: the import a census or analysis script makes."""
    load = "from src import nif_convert"
    capped = _commit_mb(load)
    uncapped = _commit_mb(load, {v: str(os.cpu_count()) for v in blas_env.BLAS_THREAD_VARS})
    margin = 50 * ((os.cpu_count() or 2) - 1)
    assert uncapped - capped > margin, (
        f"`from src import nif_convert` costs {capped} MB against {uncapped} MB "
        f"uncapped: importing the package does not cap BLAS threads before numpy")
