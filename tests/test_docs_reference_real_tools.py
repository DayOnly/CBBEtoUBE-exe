...
KNOWN_HISTORICAL = {
    "scripts/diag_jiggle_batch.py":
        "deleted; named only by 2026-08-02_AUDIT_MAIN_HISTORY.md as past work",
...
def _in_scratchpad(ref: str) -> bool:
    """Whether a tool's path INSIDE the repository sits in a scratchpad. The
    absolute path was tested before, so in any clone under a folder named
    scratchpad every tool matched and the test failed for a reason unrelated to
    the docs. #collection-side-effects"""
    return "scratchpad" in Path(ref).as_posix()
...
def test_no_runnable_tool_lives_only_in_a_gitignored_scratchpad():
    ...
    for doc, ref in _refs():
        p = PROJ / ref
        if p.is_file() and _in_scratchpad(ref):
            bad.append(f"{doc} -> {ref}")
    assert not bad, ("a doc names a tool that lives in a scratchpad:\n  "
                     + "\n  ".join(bad))


def test_the_scratchpad_rule_reads_the_repo_path_not_the_checkout(monkeypatch):
    """A clone that happens to sit under a folder named scratchpad must not turn
    every tool into a scratchpad tool. #collection-side-effects"""
    import sys
    monkeypatch.setattr(sys.modules[__name__], "PROJ", Path("C:/work/scratchpad/CBBEtoUBE-exe"))
    assert not _in_scratchpad("scripts/analysis/acceptance.py")
    assert _in_scratchpad("scratchpad/tool.py")
