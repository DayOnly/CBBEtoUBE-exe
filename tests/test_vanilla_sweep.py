...
# --- E-01: a dead worker in the end-of-run sweep must reach the user ---------

def _run_cmd_convert_with_sweep_result(tmp_path, monkeypatch, capsys, sweep_result):
    """`_cmd_convert` over one empty mod, with the end-of-run vertex-colour sweep
    stubbed to return `sweep_result`. No pool is started."""
    from src import auto_convert as ac
    ...
    def _converted(source_dir, *a, **k):
        (out / "meshes").mkdir(parents=True, exist_ok=True)   # the sweep needs it
    ...
    monkeypatch.setattr(ac.nif_convert, "sanitize_output_vertex_color_flags",
                        lambda *a, **k: dict(sweep_result))
    rc = ac._cmd_convert(_convert_ns([mod], out))
    log = capsys.readouterr().out
    assert "--- sanitizing vertex-color shader flags ---" in log, (
        "the sweep branch never ran, so nothing after this measures it")
    return rc, log
...
def test_a_dead_sweep_worker_reaches_the_log_the_tally_and_the_failures_file(
        tmp_path, monkeypatch, capsys):
    """E-01. `test_the_call_site_surfaces_it` reads this branch's SOURCE TEXT,
    so `if vc.get("pool_error") and False:` kept every string it looks for and
    passed while a worker death at the end of a run went silent again. This
    runs the branch, against a control run that differs only in that key."""
    clean = {"files": 1, "files_changed": 0, "shapes_fixed": 0, "pool_error": None}
    _, clean_log = _run_cmd_convert_with_sweep_result(
        tmp_path / "clean", monkeypatch, capsys, clean)
    clean_fails = _read_failures(tmp_path / "clean")
    _, dead_log = _run_cmd_convert_with_sweep_result(
        tmp_path / "dead", monkeypatch, capsys,
        dict(clean, pool_error="BrokenProcessPool: a worker died in the sweep"))
    dead_fails = _read_failures(tmp_path / "dead")

    assert "!! BrokenProcessPool: a worker died in the sweep" in dead_log
    assert _tally(dead_log)[1] == _tally(clean_log)[1] + 1, (
        "a dead sweep worker must add exactly one warning to the tally",
        _tally(clean_log), _tally(dead_log))
    sweep = [f for f in dead_fails if f["source"] == "vertex-colour sweep"]
    assert sweep and sweep[0]["severity"] == "warning", dead_fails
    assert not [f for f in clean_fails if f["source"] == "vertex-colour sweep"], clean_fails
