...
def test_every_gui_env_is_read_by_src():
    ...
    read = raw | helper
    # PER-IDIOM checks: an aggregate floor cannot fire when ONE idiom goes
    # blind, because the other alone clears it. Measured 2026-08-18: helper
    # 289, raw 34. Since 2026-09-16 (#env-through-helpers) every boolean and
    # numeric read goes through the helpers and the raw idiom is exactly the
    # path/string allowlist tests/test_raw_env_reads.py keeps, so the raw
    # check is an equality against that list, not a floor that a migration
    # would have to lower: helper 426, raw 10 literal names.
    from tests.test_raw_env_reads import ALLOWLIST as _RAW_ALLOWLIST
    literal_raw = {n for n in _RAW_ALLOWLIST if "{" not in n}
    assert len(helper) >= 400, (
        f"only {len(helper)} _flag/_knob env reads found across src/ -- that "
        f"idiom's regex went blind; widen it")
    assert raw == literal_raw, (
        f"the raw os.environ idiom disagrees with the allowlist by "
        f"{sorted(raw ^ literal_raw)}: a new raw read belongs in envflags, a "
        f"regex that went blind must be widened")
