...
def _FakeOsdMorph(name, offsets):
    """A real OsdMorph: the consumers read its arrays now. #osd-columnar"""
    return OsdMorph.from_offsets(name, offsets)


def _FakeOsd(morphs):
    return OsdFile(version=1, morphs=morphs)
