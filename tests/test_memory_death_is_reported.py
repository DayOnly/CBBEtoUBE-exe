...
rather than by reading the conversion code:
  ...
  2. The run log is the artefact that survives a hard kill WHOLE. It is
     line-buffered, so it holds everything up to the instant of death;
     conversion_summary.txt is written at the end of a batch and does not
     exist afterwards (since #report-checkpoint the report and the settings
     file are written from batch start on, the report marked incomplete).
     Both the CLI entry point (`open(path, "w")`) and the
...
def test_the_call_site_surfaces_it(monkeypatch):
    ...
    assert "print(" in body or "warn(" in body, (
...
def test_a_failures_file_that_cannot_be_written_is_said_out_loud(
    ...
    """E-02. Swallowing this error passed every test, and a run with failures
    would then look exactly like a clean one to anything reading the file."""
    from src import auto_convert as ac
    # A FILE where the folder must be: the atomic writer creates a missing
    # folder, so a missing one no longer fails the write. #report-checkpoint
    blocked = tmp_path / "blocked"
    blocked.write_text("a file where a folder must be", encoding="utf-8")
    monkeypatch.setattr(ac, "_failures_file_path",
                        lambda: blocked / "CBBEtoUBE_last_failures.json")
    monkeypatch.setattr(ac, "_RUN_FAILURES", [
        {"kind": "warning", "source": "vertex-colour sweep", "item": "x", "detail": ""}])
    ac._write_failures_file()
    err = capsys.readouterr().err
    assert "could not write the failures file" in err, err
    assert "1 recorded failure(s) will not appear there" in err, err
