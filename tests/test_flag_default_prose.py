...
def _claims(state: dict[str, bool]):
    ...
    # scripts/mutation_pairs.py holds PLANTED defects as data -- a pair whose
    # edit is a comment contradicting a flag's polarity (G7-i) is one -- so
    # its text is not prose about the code and is not read as a claim.
    files = (sorted((REPO_ROOT / "src").glob("*.py"))
             + sorted(p for p in (REPO_ROOT / "scripts").rglob("*.py")
                      if p.name != "mutation_pairs.py"))
    for f in files:
