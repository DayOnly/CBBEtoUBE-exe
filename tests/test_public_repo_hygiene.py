...
def test_the_local_path_rule_actually_catches_one():
    ...
    def clean(line):
        return not H.unexempt_local_path(line)
...
# `.gitattributes` says `* -text`, which PRESERVES whatever a tool writes. It
# never stopped a whole-file CRLF->LF flip from burying a few real lines under
# thousands of changed ones, and that happened to four src modules in September.


@needs_git
def test_no_crlf_in_tracked_text_outside_the_build_and_vendor_trees():
    offenders = []
    for rel in tracked:
        if not H.checks_line_endings(rel):
            continue
        try:
            raw = (REPO_ROOT / rel).read_bytes()
        except OSError:
            continue
        problem = H.line_ending_problem(rel, raw)
        if problem:
            offenders.append(problem)
    assert not offenders, "\n  ".join(
        [f"{len(offenders)} tracked text file(s) hold CRLF:"] + offenders[:30])


def test_the_line_ending_rule_actually_catches_one():
    """Control: a rule that matches nothing reads as compliance."""
    assert H.line_ending_problem("src/x.py", b"a = 1\r\nb = 2\r\n")
    assert H.line_ending_problem("docs/x.md", b"an lf line\na crlf line\r\n")
    assert H.line_ending_problem(".githooks/pre-commit", b"#!/bin/sh\r\nexit 0\r\n")
    assert H.line_ending_problem("LICENSE", b"GNU GENERAL PUBLIC LICENSE\r\n")
    assert not H.line_ending_problem("src/x.py", b"a = 1\nb = 2\n")
    assert not H.line_ending_problem("dist/CBBEtoUBE/_internal/LICENSE.txt", b"a\r\n")
    assert not H.line_ending_problem(".pynifly/pyn/x.py", b"a\r\n")
    assert not H.line_ending_problem("assets/CBBEtoUBE.ico", b"\x00\x00\r\n")
