...
def _probe_vanilla_sweep(data_dir):
    ...
    try:
        ...
        # 6-archive question.
        # No staging folder: this probe only asks contains(), and None makes
        # "writes nothing" a guarantee. It used to name a folder under %TEMP%,
        # which is inside AppData. #tool-folder-only
...
def run_checks(layout=None, *, want_overlays=False, want_overlay_copy=False,
    ...
    # #releases-link: the build being checked. It needs no modlist either, so it
    # sits above the bail with the memory row, and Check setup, `check-setup` and
    # the diagnostics zip all say which build the rest of the rows describe.
    try:
        from . import build_info as _bi
        checks.append(_c("build", "This build", OK, _bi.stamp_line()))
    except Exception as e:
        checks.append(_c("build", "This build", WARN,
                         f"could not read the build stamp ({type(e).__name__}: {e})"))
    ...
    # The settings file itself. A torn file loads as DEFAULTS by design (the GUI
    # has to open), and the next save writes those defaults over the user's
    # settings -- so say it here, before a run or a save. #run-warnings
    try:
        from . import build_info as _bi
        _sf = _bi.settings_file_status()
        if _sf.get("status") == "malformed":
            checks.append(_c(
                "settingsfile", "Settings file", WARN,
                f"{_sf.get('path')} could not be read, so every option is at its "
                "default right now.",
                "Close the tool, replace the file with CBBEtoUBE_settings.json.bak "
                "from the same folder, and start it again. Saving settings now "
                "would write the defaults over yours."))
        else:
            checks.append(_c(
                "settingsfile", "Settings file", OK,
                "none saved yet -- every option is at its default"
                if _sf.get("status") == "absent" else f"{_sf.get('path')} read"))
    except Exception:
        pass
    ...
    try:
        ...
        if _new:
            ...
            checks.append(_c(
                ...
                "Review them on the GUI's tabs and choose them, or keep the defaults: "
                "any settings change saves, and that save records them and clears this."))
        elif not _baseline:
            checks.append(_c(
                "newsettings", "New options in this build", WARN,
...
def format_checks(checks) -> "list[str]":
    """One line per check -- `[status] label: detail  | fix: ...` -- exactly as the
    diagnostics zip's preflight.txt and `CBBEtoUBE.exe check-setup` print them.
    One formatter, so the two cannot drift. #check-setup"""
    return [f"[{c.status}] {c.label}: {c.detail}" + (f"  | fix: {c.fix}" if c.fix else "")
            for c in checks]
