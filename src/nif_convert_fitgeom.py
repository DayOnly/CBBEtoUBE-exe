...
from .nif_convert_bodyrefs import _find_ube_body_osd  # noqa: E402
from .nif_convert_telemetry import _note_pass_failure  # noqa: E402
from .nif_convert_trigen import _cached_body_morph_stack  # noqa: E402
...
ANTIPOKE_BUST_CLEAR = _knob("CBBE2UBE_BUST_CLEAR", 1.0)
    ...
    if (min_standoff > 0
        ...
        if need_push.any():
            ...
            if _nc().WARP_STANDOFF_SMOOTH_ENABLED and tris is not None and moved.any():
                ...
                _sm_d, sm_i = ube_tree.query(warped, k=1)
                _un_d, un_i = ube_tree.query(unsmoothed, k=1)
    ...
    if np.any(in_bust):
        ...
        if req_back is not None and in_back.any():
            ...
            if _hit < _nc().BACK_MIN_VERTS:
                # One line per piece, so a run is auditable the way min-push
                # is. Unconditional since 2026-09-16: the verbosity switch
                # that could silence it had no reader, no test and no record,
                # and was retired (CHANGELOG names it). #kill-switch-verdicts
...
def _bust_morph_chord_req(cur_cloth, tris, ui, ube_body_verts,
    ...
    whose sum-of-positive-residuals variant was built and measured WORSE (the
    reference preset: -0.082 -> -0.279u, 138 poking). Presets do add, but the
    supremum over
...
def _solve_clearance_field(verts, normals, need, tris, *,
    ...
    try:
        ...
        stats["ok"] = True
        if _flag("CBBE2UBE_FIELD_STATS", False):
