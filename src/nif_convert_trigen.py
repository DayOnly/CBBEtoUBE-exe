...
from .atomic_io import atomic_tri_save
...
def _cached_body_morph_amplitude(osd_path: Path,
    ...
    for m in osd.morphs:
        ...
        if not any(k in nm for k in _MORPH_SIZE_KEYWORDS):
            continue
        ii = m.idx.astype(np.int64)
        ok = ii < n_verts
        if not ok.any():
            continue
        ii = ii[ok]
        d = m.delta[ok].astype(np.float64)
        # The scalar loop this replaced summed dx*n0 + dy*n1 + dz*n2 left to
        # right in float64. The same three products and two sums in that order
        # give the same bits; an einsum does not (1.78e-15 apart on the real
        # body). #osd-columnar
        outward = d[:, 0] * bn[ii, 0] + d[:, 1] * bn[ii, 1] + d[:, 2] * bn[ii, 2]
        np.maximum.at(amp, ii, outward)
