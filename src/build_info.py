...
* `stamp()`      -- version, git commit (+dirty), build time, whether frozen,
                    ...
                    from `src/_build_stamp.py`, which `CBBEtoUBE.spec` writes
                    through `scripts/build_identity.py` at the start of every
                    build, with the toolchain versions (gitignored; absent in
...
try:                                    # written by CBBEtoUBE.spec at build time
...
def stamp() -> dict:
    ...
    return {
        ...
        "toolchain": _BUILD.get("toolchain"),
        "frozen": frozen,
        "exe": exe,
...
def write_run_config(output_dir, name: str = "conversion_settings.json", *,
    ...
    """Drop the attribution block next to conversion_summary.txt. `workers` as
    for run_config(): the pool that ran, so the sidecar names it rather than a
    count re-derived at write time. #report-checkpoint"""
    import json
    try:
        from .atomic_io import atomic_write_bytes
        p = Path(output_dir) / name
        atomic_write_bytes(p, json.dumps(run_config(workers=workers), indent=2,
