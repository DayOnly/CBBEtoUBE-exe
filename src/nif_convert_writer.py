...
def _normalize_partitions_on_disk(dst_path: Path,
    ...
    try:
        ...
        for s in nf.shapes:
            ...
            if len(getattr(s, "bone_names", []) or []) > SKIN_PARTITION_BONE_CAP:
                ...
                if _nc()._split_oversize_partition(
                    ...
                    import sys as _sys
                    warn(f"{s.name} has {len(s.bone_names)} bones (> {SKIN_PARTITION_BONE_CAP}"
                         "-bone GPU cap) and could NOT be split into partitions",
            ...
            if len(getattr(s, "verts", []) or []) > _nc().SKIN_PARTITION_VERT_CAP:
                if _nc()._split_oversize_partition_verts(s, part_id=keep_slot) > 1:
                    ...
                    import sys as _sys
                    warn(f"{s.name} has {len(s.verts)} verts (> {_nc().SKIN_PARTITION_VERT_CAP}"
        ...
        warn(f"partition normalize FAILED: {plain_error(_pe)}",
...
def sanitize_output_vertex_color_flags(meshes_root, workers: "int | None" = None) -> dict:
    ...
    from .child_lifetime import tie_children_to_this_process
    # A fresh pool at the very end of a run: its workers must die with this
    # process too, or a kill during the sweep leaves them running.
    # #workers-die-with-parent
...
def _reauthor_nif_fresh(dst_path: Path, override_verts_by_name=None,
    ...
    try:
        ...
        if copy_failed:
            ...
            import sys as _sys
            warn(f"re-author of {dst_path.name} dropped shape(s) "
