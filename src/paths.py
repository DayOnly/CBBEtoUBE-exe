...
def tool_dir() -> Path:
    """The tool's own folder: next to the exe when frozen, the repo root when run
    from source.

    EVERYTHING THE TOOL KEEPS LIVES HERE, AND NOWHERE ELSE. #tool-folder-only
    Settings, exclusions and logs sit beside the exe; nothing goes under AppData,
    and the Windows temp folder is inside AppData too. Replacing this folder
    resets the tool completely -- a user decision (2026-09-15), taken after a
    user replaced the folder and found their exclusions still waiting."""
