...
# The update channel is the browser: the exe ships no ssl and never goes online.
# #releases-link
...
def _run_stats(report: dict | None) -> list[str]:
    ...
    # A checkpoint from a run that died, not a finished run. #report-checkpoint
    if report.get("complete") is False:
        lines.insert(0, f"  RUN DID NOT FINISH: {n('source_mods')} of "
                        f"{n('sources_planned')} planned mods, then it stopped")
...
def build_report(version: str,
    ...
    if kind == "conversion":
        # REPORTING.md: for clipping, standing still versus moving changes the
        # diagnosis more than anything else, and some problems only happen at a
        # particular slider and weight. The issue form asks both; so does this.
        add("  standing still / only in motion / only when zoomed out: <which>")
        add("  body preset and weight: <preset, weight>")
    add("")
