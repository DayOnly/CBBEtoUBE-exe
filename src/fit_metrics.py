...
CAST_CULL = not _flag("CBBE2UBE_NO_CAST_CULL", False)
...
RAY_CHUNK = _knob("CBBE2UBE_RAY_CHUNK", 512, int)

MIN_HITS = 40          # below this the garment does not cover the bust and
                       # the distribution is a handful of stray rays
CEIL_MEDIAN = _knob("CBBE2UBE_STANDOFF_CEIL_MEDIAN", 1.60)
CEIL_P90 = _knob("CBBE2UBE_STANDOFF_CEIL_P90", 2.20)


def _enabled() -> bool:
    return not _flag("CBBE2UBE_NO_STANDOFF_AUDIT", False)
...
def _band_enabled() -> bool:
    ...
    if not _enabled():
        return False
    return _flag("CBBE2UBE_STANDOFF_BAND_AUDIT", False)
...
PUSH_Z_LO = _knob("CBBE2UBE_PUSH_Z_LO", float(BAND_Z[0] - 4.0))
PUSH_Z_HI = _knob("CBBE2UBE_PUSH_Z_HI", float(BAND_Z[1]))
PUSH_Y_MIN = _knob("CBBE2UBE_PUSH_Y_MIN", -2.0)
...
# to 0.
PUSH_RIM_MARGIN = _knob("CBBE2UBE_PUSH_RIM_MARGIN", 2.0)
...
PUSH_MAX_REACH = _knob("CBBE2UBE_PUSH_MAX_REACH", 3.0)
PUSH_ITERS = _knob("CBBE2UBE_PUSH_ITERS", 8, int)
PUSH_DAMP = _knob("CBBE2UBE_PUSH_DAMP", 0.5)
PUSH_KNN = _knob("CBBE2UBE_PUSH_KNN", 3, int)
PUSH_MARGIN = _knob("CBBE2UBE_PUSH_MARGIN", 0.05)
PUSH_MAX_TOTAL = _knob("CBBE2UBE_PUSH_MAX_TOTAL", 2.00)
...
PUSH_REQ_CAP = _knob("CBBE2UBE_PUSH_REQ_CAP", 1.50)
PUSH_MIN_REGION = 50          # too little covered skin to judge
PUSH_ENABLED = not _flag("CBBE2UBE_NO_MIN_PUSH", False)
...
PASS_TRACE = _flag("CBBE2UBE_PASS_TRACE", False)
...
SURVIVAL_TRACE = _flag("CBBE2UBE_SURVIVAL_TRACE", False)
...
CHAIN_GUARD = not _flag("CBBE2UBE_NO_CHAIN_GUARD", False)
CHAIN_TOL = _knob("CBBE2UBE_CHAIN_TOL", 0, int)
...
STANDOFF_TRACE = _flag("CBBE2UBE_STANDOFF_TRACE", False)
