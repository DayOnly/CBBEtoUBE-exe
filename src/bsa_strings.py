...
class BSAArchive:
    ...
    def __init__(self, path: str | Path, eager: bool = True):
        ...
        # on large BSAs; read_file() then reads just that entry's bytes from
        # disk and never loads the archive. #bsa-seek-read
        self._eager = eager
    ...
    def read_file(self, name: str) -> bytes | None:
        ...
        # Bounds-check the file-supplied offset/size UNCONDITIONALLY, in both
        # modes: a malformed BSA whose record offset points past EOF must return
        # None, never OOB-read (IndexError / struct.error) from a crafted archive.
        if self._eager:
            d, base = self._data, 0
            if off < 0 or size < 0 or off + size > len(d):
                return None
        else:
            # TABLE-ONLY: seek to this entry and read only its bytes. This used
            # to load the whole archive and keep it -- measured on the 1.15 GB
            # base-game mesh archive, reading one 35 KB mesh added 1,100 MB of
            # private memory. #bsa-seek-read
            try:
                with open(self.path, "rb") as f:
                    end = f.seek(0, 2)
                    if off < 0 or size < 0 or off + size > end:
                        return None
                    f.seek(off)
                    # + room for an embedded-name prefix (a length byte and up
                    # to 255 characters) ahead of the entry's data
                    d = f.read(min(end - off, size + 256))
            except OSError:
                return None
            base = off
        p = off - base
...
class StringResolver:
    ...
    def _interface_bsa(self) -> BSAArchive | None:
        if not self._bsa_loaded:
            ...
            if p.is_file():
                try:
                    self._bsa = BSAArchive(p, eager=False)   # #bsa-seek-read
                except Exception:
                    self._bsa = None
        return self._bsa
