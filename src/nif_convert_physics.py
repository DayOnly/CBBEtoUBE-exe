...
from .nif_convert_fitgeom import warp_armor_by_body_delta  # noqa: E402
...
def _add_butt_collider_patch(dst_path) -> int:
    ...
    if not ok:
        try:
            ...
            _restore = "RESTORED original"
        except Exception as _re:
            # The WARN below used to say RESTORED either way. #silent-xml-write
            _note_pass_failure("_add_butt_collider_patch/restore", _re, p)
            _restore = "restore FAILED, the unverified patched NIF is still in place"
        print(f"  WARN: butt-collider patch on {p.name} lost shapes/extra-data "
              f"-- {_restore}, patch skipped", file=sys.stderr)
        return 0
...
def _add_skirt_collider_proxy(dst_path) -> int:
    ...
    if not ok:
        try:
            ...
            # The WARN below used to say RESTORED either way. #silent-xml-write
            _note_pass_failure("_add_skirt_collider_proxy/restore", _re, p)
            _restore = "restore FAILED, the unverified patched NIF is still in place"
        print(f"  WARN: skirt proxy on {p.name} lost shapes/extra-data -- "
              f"{_restore}, proxy skipped", file=sys.stderr)
        return 0
...
def _make_chains_static(xml_path: Path) -> None:
    ...
    if t2 != t:
        try:
            ...
            # The chains keep their mass: the piece ships with the old physics.
            # #silent-xml-write
            _note_pass_failure("_make_chains_static/xml-write", _we, xml_path)
...
def _ensure_cloth_body_collider(xml_path: Path, nif) -> bool:
    ...
    try:
        ...
        _note_pass_failure("_ensure_cloth_body_collider/xml-write", _we, xml_path)
        return False
...
def _harden_hdt_xml_for_fsmp(xml_path: Path, nif) -> None:
    ...
    if changed:
        try:
            ...
            # The unresolvable blocks stay in the XML FSMP loads. #silent-xml-write
            _note_pass_failure("_harden_hdt_xml_for_fsmp/xml-write", _we, xml_path)
...
def _finalize_hdt_physics(dst_path: Path, src_nif_path: Path) -> bool:
    ...
    try:
        ...
        try:
            ...
            _note_pass_failure("_ensure_cloth_body_collider", _be, dst_path)
        ...
        try:
            ...
            _note_pass_failure("_harden_hdt_xml_for_fsmp", _he, dst_path)
        ...
        if _flag("CBBE2UBE_DEBUG_FINALIZE", False):
