...
SETTINGS: "tuple[Setting, ...]" = (
    ...
    # ---- documented in README, and until now reachable ONLY by environment
    # variable -- which an MO2 launch never passes on. #documented-toggle-rows
    Setting("bust_spacing_aware",
            "Size the bust anti-poke by the garment's own spacing",
            "Armor", "Fit and clearance", default=True,
            env="CBBE2UBE_NO_BUST_SPACING", invert=True, advanced=True,
            hint="Untick to go back to the old fixed search radius.",
            tooltip="The bust anti-poke pushes per garment vertex but used a fixed "
                    "neighbourhood. On a 3339-vertex cuirass whose vertices sit "
                    "0.71-1.21u apart, the body tip poking BETWEEN two of them fell "
                    "outside the 0.673u the pass actually looked at: it delivered "
                    "0.125u of clearance against a 0.55u requirement, with nothing "
                    "in the pass aware it had missed. Sizing the neighbourhood by "
                    "each vertex's own spacing reaches the tip."),
    Setting("bust_morph_residual",
            "Cover what the body's sliders still demand",
            "Armor", "Fit and clearance", default=True,
            env="CBBE2UBE_NO_BUST_MORPH_RESIDUAL", invert=True, advanced=True,
            hint="Untick if a body preset leaves the bust sitting too far off.",
            tooltip="After the fit, a body slider can still ask for more room over "
                    "the bust than the garment has. This charges that residual, up "
                    "to a 1.5u ceiling, so a piece stays covered on presets it was "
                    "never fitted against. The counter-metric is nipple "
                    "poke-through: too little room there is worse than an outline."),
    Setting("softcloth_inflate",
            "Clear soft-body cloth over the bust and butt",
            "Armor", "Physics", default=True,
            env="CBBE2UBE_NO_SOFTCLOTH_INFLATE", invert=True, advanced=True,
            hint="Untick only if simulated cloth sits too far off the body.",
            tooltip="The main anti-poke SKIPS soft-body and HDT-rigged shapes, "
                    "because moving every vertex disturbs the simulation -- so the "
                    "larger UBE body pokes through physics cloth the rest of the fit "
                    "never touches. This pushes outward only, band-limited to the "
                    "breast and butt, and leaves the rest of the simulated rest "
                    "shape alone. Confirmed in game on a fur-collared cuirass."),
    Setting("groove_onesided",
            "Groove cleanup may not pull cloth onto the skin",
            "Armor", "Fit and clearance", default=True,
            env="CBBE2UBE_GROOVE_ONESIDED", advanced=True,
            hint="Untick to restore the older two-sided smoothing for comparison.",
            tooltip="Groove cleanup smooths the displacement field, and that field "
                    "PEAKS at the bust apex, so smoothing flattened the peak and "
                    "pulled the garment back onto the skin -- hardest exactly where "
                    "that does the most damage (measured: 0.191u of displacement "
                    "lost on the newly exposed vertices). Removing only the INWARD "
                    "component keeps the tangential and outward motion, which is "
                    "what actually flattens a groove."),
