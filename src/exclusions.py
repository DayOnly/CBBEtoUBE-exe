...
def config_path() -> Path:
    """Where the exclusions JSON lives: in the tool's own folder, beside
    CBBEtoUBE_settings.json. CBBE2UBE_EXCLUSIONS overrides.

    IN THE TOOL FOLDER, AND NOWHERE ELSE. #tool-folder-only
    From 1.1 to 1.4.1 this file lived in a CBBEtoUBE folder under
    %LOCALAPPDATA%, so a mirror-copy redeploy could not wipe it. That also meant
    replacing the tool folder did NOT reset a user's exclusions -- reported by a
    user who did exactly that. The tool now keeps nothing outside its own folder.

    The old AppData file is deliberately never read, moved or created: an
    upgrade starts with no exclusions, and the Run tab's per-session review gate
    makes the user look at the list before an All-mods run. Redeploys keep this
    file because scripts/deploy_exe.ps1 copies with robocopy /E and skips it."""
    override = os.environ.get("CBBE2UBE_EXCLUSIONS", "").strip()
    if override:
        return Path(override)
    return _paths.tool_dir() / "CBBEtoUBE_exclusions.json"
