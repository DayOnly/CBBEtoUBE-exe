...
#
# The key includes mtime+size, so a rewritten ESP invalidates itself: clearing is
# never needed for CORRECTNESS. It is needed for MEMORY where a long-lived process
# parses a whole load order and keeps only a summary -- the GUI's mod-list refresh
# (auto_convert.list_convertible_mods). #esp-cache-release
_LOAD_CACHE: "dict[tuple, ESP]" = {}


def clear_load_cache() -> int:
    """Drop every cached parse; returns how many there were."""
    n = len(_LOAD_CACHE)
    _LOAD_CACHE.clear()
    return n
