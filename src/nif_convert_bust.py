...
from .atomic_io import atomic_nif_save, atomic_write_bytes
...
def _split_bust_collider_shape(dst_path, src_path=None) -> int:
    ...
    if not ok:
        try:
            ...
            _note_pass_failure("_split_bust_collider_shape/restore", _re, dst_path)
        ...
        print(f"  WARN: bust-collider split on {Path(dst_path).name} lost "
              f"shapes/extra-data -- {_restore}, split skipped",
              file=sys.stderr)
        return 0
    for n in added:
...
def _split_bust_collider_xml(dst_path) -> int:
    ...
    if n_rw:
        try:
            ...
            # The collider stays on the garment, not its clone. #silent-xml-write
            _note_pass_failure("_split_bust_collider_xml/xml-write", _we, xml)
            return 0
