...
def _overlay_workers() -> int:
    ...
    n = _knob("CBBE2UBE_OVERLAY_WORKERS", 0, int)
    if n > 0:
        return min(n, 32)
...
def convert_overlays(output_dir, layout, *, regions=("body", "hands", "feet"),
    ...
    work = _new_work_dir(out_root, "ube_overlay_")
    arc_cache: dict = {}
...
# WORK FOLDERS GO IN THE OUTPUT MOD, NOT %TEMP%. #tool-folder-only
# The temp folder is under AppData and the tool keeps nothing there. Each pass
# makes its folder under `<output>/_overlay_work/` and removes it when it ends,
# the way the mesh pass stages archive meshes in `<output>/_bsa_staging`.
_OVERLAY_WORK = "_overlay_work"


def _new_work_dir(out_root, prefix: str) -> Path:
    """A fresh, empty work folder for one pass, inside the output mod."""
    import tempfile
    parent = Path(out_root) / _OVERLAY_WORK
    parent.mkdir(parents=True, exist_ok=True)
    return Path(tempfile.mkdtemp(prefix=prefix, dir=str(parent)))


def _drop_work_dir(work) -> None:
    """Remove one pass's work folder, and the shared parent once it is empty."""
    import shutil
    work = Path(work)
    shutil.rmtree(work, ignore_errors=True)
    try:
        work.parent.rmdir()        # fails, harmlessly, while a sibling remains
    except OSError:
        pass


def _compile_psc(compiler, psc_path, src, basesrc, out_dir, work_dir):
    """Compile one .psc to .pex, written ATOMICALLY into out_dir. Compiles into a
    private folder under `work_dir` first, then atomic_write_bytes the .pex into out_dir, so a
    ...
    tmp_out = tempfile.mkdtemp(prefix="ube_pex_", dir=str(work_dir))
    try:
        r = subprocess.run(
...
def build_multislot_feet_overlays(layout, out_root, texconv, *, skip_mods=(),
    ...
    work = _new_work_dir(out_root, "ube_feet_")
    try:
        ...
        for name, text in _iter_feetpaint_scripts(layout, skip_mods=skip_mods):
            ...
            epsc.write_bytes(edited.replace("\r\n", "\n").replace("\r", "\n")
                             .encode("utf-8"))
            pex, clog = _compile_psc(compiler, epsc, src, basesrc, pex_out, work)
            n_scripts += 1
            if pex:
                n_compiled += 1
            else:
...
def add_ube_overlay_copies(layout, out_root, texconv, *,
    ...
    work = _new_work_dir(out_root, "ube_ovcopy_")
    try:
        src, basesrc = _assemble_papyrus_imports(compiler, work)
        if src is None:
