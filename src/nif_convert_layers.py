...
from .nif_convert_fitgeom import _panel_rigid_disp  # noqa: E402
        ...
        for _round in range(2):
            ...
            for a in shape_order:
                ...
                push = np.minimum(
                    ...
                    np.minimum(headroom, allowed))
                if _flag("CBBE2UBE_LAYER_DEBUG", False):
...
_SEAM_WELD_TOL = _knob("CBBE2UBE_SEAM_WELD_TOL", 0.05)
...
def _weld_cross_shape_seams(shape_jobs, tol: float = _SEAM_WELD_TOL,
    ...
    if seam_clusters and not _flag("CBBE2UBE_NO_SEAM_SKIN_MATCH", False):
        try:
            ...
            if _flag("CBBE2UBE_SEAM_DEBUG", False):
                import sys as _sys
...
_LAYER_RIDE_MAX = _knob("CBBE2UBE_LAYER_RIDE_MAX", 2.0)

_LAYER_ORDER_NEAR = _knob("CBBE2UBE_LAYER_ORDER_NEAR", 2.0)

_LAYER_ORDER_ITERS = _knob("CBBE2UBE_LAYER_ORDER_ITERS", 2, int)
...
_GLOW_RIDE_MAX = _knob("CBBE2UBE_GLOW_RIDE_MAX", 2.0)
...
def _ride_effect_overlays_on_plate(shape_jobs, ride_max: float = _GLOW_RIDE_MAX):
    ...
    if _flag("CBBE2UBE_GLOW_RIDE_DEBUG", False):
        import sys as _sys
