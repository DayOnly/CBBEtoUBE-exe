...
from .atomic_io import atomic_nif_save, atomic_copy, atomic_tri_save
...
from .nif_convert_trigen import (  # noqa: E402
    _BODY_MORPH_STACK_CACHE,
    _BODY_MORPH_DIFF_CACHE,
    _cached_osd_load,
    _cached_body_morph_amplitude,
    _cached_body_morph_differential,
    _tri_is_owning_variant,
    _tri_fits_variant,
    pair_alias_map,
    _reset_morph_flags,
    armor_relpath_under_meshes,
    _collect_tri_inputs,
    _normalize_shader_for_morph,
    _pick_bodytri_carriers,
    _source_morph_tri_shape_names,
...
# ("file", path, size, mtime_ns, shape name, n verts, leg_region_only, excl) ->
# (None, scale_bones, {bone: (verts, cKDTree, wts)}) for a body read from a NIF on
# disk; (id(body_shape), leg_region_only, excl) -> (body_shape, ...) for anything
# else. See _cached_scale_bone_data (#scale-bone-cache-by-file, #idreuse-cache).
...
from .nif_convert_fitgeom import (  # noqa: E402
    ...
    ADAPTIVE_CLEARANCE_MORPH_FACTOR,
    ADAPTIVE_CLEARANCE_MORPH_MAX,
    _authored_floor_amp_room,
    _authored_normals_usable,
    _feathered_authored,
    ANTIPOKE_BUST_CLEAR,
    JIGGLE_CLEARANCE_GAIN,
    JIGGLE_CLEARANCE_MAX,
    CONFORM_BUST_CLEARANCE,
    STATIC_AUTHORED_AMP,
    STATIC_AUTHORED_MIN_CLEARANCE,
    warp_armor_by_body_delta,
    _damp_to_avoid_inversion,
    _limit_triangle_shear,
    _smooth_warp_grooves,
    _uniformise_local_scale,
    _local_edge_length,
    _cap_short_edge_stretch,
    _inflate_cloth_over_bust_butt,
    conform_to_source_standoff,
    _surface_deficit,
    _bust_morph_chord_req,
    _relax_conform_field,
    _solve_clearance_field,
    _reach_iters,
    _reach_screen,
    clear_armor_outside_body,
    _smooth_push_field,
    snap_armor_outside_body,
    repair_collapsed_tris,
...
# docs/worklog/2026-09-04_NIPPLE_OUTLINE_THROUGH_PLATE.md.
#
...
SMP_ANTIPOKE_MAX_PUSH = _knob("CBBE2UBE_SMP_ANTIPOKE_PUSH", float(ANTIPOKE_BUST_CLEAR))
...
from .nif_convert_weights import (  # noqa: E402
    ...
    RESKIN_K,
    RESKIN_NEAR_DIST,
    _match_limb_motion_to_body,
    _match_leg_motion_to_body,
    _match_arm_motion_to_body,
    _match_spine_motion_to_body,
    _match_spine_twist_to_body,
    ...
    _conform_fitted_to_body,
    _conform_weights_core,
    _cap_skin_bone_count,
    _cap_weights_map,
    _cap_and_renormalise_rows,
    _fill_zero_weight_verts,
    _set_override_vert_weights,
    _weights_to_index,
    _weights_from_index,
    _strip_genital_weights_map,
    _strip_jiggle_weights_map,
    add_scale_bone_weights,
    compute_body_blend_skinning,
    _slot_aware_reskin_band,
    _slot_aware_scale_bone_reach,
    _sync_chest_layered_cloth_weights,
    _sync_abdomen_layered_cloth_weights,
    _source_bust_weight_map,
    _body_jiggle_weight,
    _body_breast_motion_weight,
    _jiggle_region_of,
    _jiggle_transfer_vert,
    _chest_match_vert,
    _chest_match_strength,
    _butt_match_vert,
    _butt_match_strength,
    _leg_deform_match_vert,
    _cached_scale_bone_data,
    _align_scale_bone_stbs_to_verts,
    _adjust_skin_to_bone_baked,
    _is_scale_bone,
    _is_breast_bone,
    _is_leg_rigid_bone,
    _is_physics_jiggle_scale_bone,
    _is_soft_body_physics_bone,
    _is_genital_anatomy_bone,
    _is_arm_hand_bone,
    _is_physics_evidence_bone,
    _is_skeleton_bone,
)
...
# no -- "we can't use that in its current state". It was set back to default OFF
# that day, to wait for the fold regression to be designed out, not tuned away;
# the 2026-09-09 note below records why it is default ON now.
#
# The mechanism stays fixed and armable so the next attempt starts from a floor
# that works rather than re-deriving why it never did. Both halves are armed
# together by default now; to measure without them, disarm BOTH
# (`CBBE2UBE_NO_AUTHORED_INFLATE=1 CBBE2UBE_NO_AUTHORED_ANTIPOKE=1`) and read
# the table below before drawing anything from a single-flag arm.
#
...
# is why the default went back off on 2026-09-05.
#
...
#
# WHY IT MATTERS. Measured 2026-08-27, pack census on the bust band under the
# reference UBE preset (768 examined / 257 scored, 0 error exclusions): the
# "clean at bind but clipping morphed" class is 28% of body-swap pieces AND 28%
# of copy-path
...
# no-op. It was DEFAULT OFF pending an A/B and an in-game verdict, until the
# promotion below.
# PROMOTED TO DEFAULT ON 2026-09-09, to match what actually ships. The
# deployed recipe has set this since before the 2026-09-06 reconvert, so
# the pack has been built with it ON while the code said OFF -- and the
...
#
# Was DEFAULT OFF until measured. F010 is the standing caution for this area:
...
# game. docs/worklog/2026-09-02_BUTT_COPY_PATH_TROUSERS.md
#
...
WARP_STANDOFF_SMOOTH_ITERS = _knob("CBBE2UBE_WARP_SMOOTH_ITERS", 2, int)
...
WARP_DELTA_SMOOTH_ITERS = _knob("CBBE2UBE_WARP_DELTA_SMOOTH", 0, int)
WARP_DELTA_SMOOTH_WEIGHT = _knob("CBBE2UBE_WARP_DELTA_SMOOTH_W", 0.5)
...
from .nif_convert_bodyrefs import (  # noqa: E402
    _BODY_DISCOVERY_CACHE,
    _find_cbbe_base_body,
    weight_suffix_of,
    _find_ube_femalebody,
    _sorted_mod_dirs,
    _glob_first_in_mods,
    _find_ube_shapedata,
    _find_ube_template_body,
    _find_ube_body_osd,
...
from .nif_convert_writer import (  # noqa: E402
    ...
    _repair_coherence_collapse,
    _repair_degenerate_normals,
    _repair_effect_shader_shape_controllers,
    _repair_winding_consistency,
    _close_pubic_holes,
    _drop_scale_bones_from_skin,
    _geometry_repair_allowed,
...
# Read through the shared flag reader, like every other toggle: a raw environ read
# is invisible to the flag census (which parses `_flag(` calls) and to the GUI,
# which is why this one had no row while the README documented it.
# #documented-toggle-rows. Unset keeps it enabled and "0" disables it, as before;
# only an odd value moves -- "2" used to enable it and now disables it, which is
# what it already did for every other flag.
GROOVE_ONESIDED = _flag("CBBE2UBE_GROOVE_ONESIDED", True)
...
from .nif_convert_bust import (  # noqa: E402
    _body_nipple_weight,
    _bust_split_candidates,
    _bust_split_xml_text,
    _chest_follow_target,
    _split_bust_collider_shape,
    _split_bust_collider_xml,
...
FIT_STAGES = (
    ...
    ("snap",                "snap_armor_outside_body",    ("copy", "swap"),
     ...
     "docs/worklog/2026-09-02_BUTT_COPY_PATH_TROUSERS.md"),
    ...
    if not warp_armor:
        ...
        if STATIC_CHAINS:
            try:
                ...
                _note_pass_failure("_make_chains_static", _se, dst_path)
...
def fit_armor_to_ube_body(
    ...
    *,
    ...
    docs/M3_findings.md" — a file that was never written (the belt script
    that cited it, tests/test_m3_belt.py, said "when written"; pytest never
    collected it and it went on 2026-09-16, its byte-identity premise long
    lapsed). A cited safety net that is not
    ...
    if (AUTHORED_INFLATE and body_normals is not None
        ...
        try:
            ...
            if sa.shape == armor_verts.shape and sb.shape == sn.shape and len(sb):
                ...
                # everywhere else.
                if _flag("CBBE2UBE_NIPPLE_PROBE", False):
...
from .nif_convert_physics import (  # noqa: E402
    ...
    _hdt_xml_bind_piece_source,
    _hdt_xml_cache_clear,
    _lift_chain_roots_off_body,
    _nearest_declared_ancestor,
    _norm_bone,
    _precreate_custom_bone_chains,
...
# --- #body-lookup-prefers-baseshape -- DEFAULT ON since 2026-09-04 -----------
#
...
#
# It was OFF at first because it changes the TRI on every physics piece, which
# is a change to how bodies morph in game and needed its own verdict.
...
# docs/worklog/2026-09-02_ZEROWEIGHT_BONE_PRODUCER.md. On a 38-NIF outfit (38 NIFs, 294
...
# docs/worklog/2026-09-02_ZEROWEIGHT_BONE_PRODUCER.md. This comment used to read "it adds no
...
# docs/worklog/2026-09-02_ZEROWEIGHT_BONE_PRODUCER.md), so the fit cost is bounded by
...
_SELFINT_SMOOTH = _knob("CBBE2UBE_SELFINT_SMOOTH", 4, int)
...
# discontinuity.
_COINCIDENT_SKIN_GATE = _knob("CBBE2UBE_COINCIDENT_SKIN_GATE", 0.10)
# Fraction of EVERY member's row the shared palette must carry.
_COINCIDENT_SKIN_MIN_SHARE = _knob("CBBE2UBE_COINCIDENT_SKIN_MIN_SHARE", 0.75)
...
_RIGID_PART_GATE = _knob("CBBE2UBE_RIGID_PART_GATE", 0.15)
# Below this a "part" is a stray sliver, not an ornament worth rigidifying.
_RIGID_PART_MIN_VERTS = _knob("CBBE2UBE_RIGID_PART_MIN_VERTS", 8, int)
...
_AUTHOR_DEV_MARGIN = _knob("CBBE2UBE_AUTHOR_DEV_MARGIN", 0.15)
...
_AUTHOR_DEV_MIN_PALETTE = _knob("CBBE2UBE_AUTHOR_DEV_MIN_PALETTE", 0.5)
...
# costs cord quality). Net-positive but NOT clean, so it was kept opt-in rather than
...
_LAYER_RIDE_K = _knob("CBBE2UBE_LAYER_RIDE_K", 8, int)
...
_LAYER_RIDE_BARY_CAND = _knob("CBBE2UBE_LAYER_RIDE_BARY_CAND", 6, int)
...
_LAYER_ORDER_SMOOTH = _knob("CBBE2UBE_LAYER_ORDER_SMOOTH", 2, int)
...
PANEL_RIGIDITY_MIN_VERTS = _knob("CBBE2UBE_PANEL_RIGIDITY_MIN_VERTS", 24, int)
...
# docs/worklog/2026-09-04_NIPPLE_OUTLINE_THROUGH_PLATE.md). On the reported piece the
...
# RE-EXPORTS. #unused-imports
# These names are imported above from the split modules and used by NO code
# in this file; tests and callers reach them as `nc.<name>` (and the split
# modules as `_nc().<name>`). `__all__` is the one re-export form pyflakes
# honours, so the zero-unused-imports gate in tests/test_no_undefined_names.py
# can hold. Found by scanning src/, tests/, scripts/ and research/ for
# `nc.<name>`, `_nc().<name>`, from-imports and getattr/setattr strings.
# Nothing here is imported with `*`.
__all__ = [
    "CONFORM_BUST_CLEARANCE",
    "JIGGLE_CLEARANCE_GAIN",
    "JIGGLE_CLEARANCE_MAX",
    ...
    "_ColliderDeclined",
    "_PASS_EFFECTS",
    "_PASS_EFFECTS_THIS_PIECE",
    "_PASS_FAILURES",
    "_PASS_FAILURES_THIS_PIECE",
    "_actor_can_resolve_bone",
    "_actor_skeleton_bone_parents",
    "_adjust_skin_to_bone_baked",
    "_align_scale_bone_stbs_to_verts",
