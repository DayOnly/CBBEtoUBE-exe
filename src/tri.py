...
IN MEMORY a morph is two arrays (an int64 index and a float64 delta per
offset), not a tuple per offset -- the same change as the OSD's, and for the
same reason: the generator builds these from arrays and the writer quantises
them from arrays, and a tuple in between was a copy that served nothing.
The values are what the tuples held, float64, so the bytes written do not
change. #osd-columnar
"""
...
# One stored offset as the file lays it out: uint16 vertex index, three int16
# quantised deltas -- the "<H3h" record, 8 bytes.
TRI_RECORD = np.dtype([("idx", "<u2"), ("q", "<i2", (3,))])
assert TRI_RECORD.itemsize == 8


@dataclass(eq=False)
class TriMorph:
    """One named morph for one shape -- a sparse table of vertex deltas.

    Position k moves vertex idx[k] by delta[k]. Stored quantized on disk;
    here the deltas are real float64 values (multiplier applied)."""
    name: str
    idx: np.ndarray      # (n,) int64
    delta: np.ndarray    # (n, 3) float64

    def __post_init__(self) -> None:
        self.idx = np.ascontiguousarray(
            np.asarray(self.idx, dtype=np.int64)).reshape(-1)
        self.delta = np.ascontiguousarray(
            np.asarray(self.delta, dtype=np.float64)).reshape(-1, 3)
    ...
    def parse(cls, data: bytes) -> "TriFile":
        ...
        while p < len(data) - 3:
            ...
            for _ in range(num_morphs):
                ...
                if noff:
                    rec = np.frombuffer(data, dtype=TRI_RECORD, count=noff, offset=p)
                    idx = rec["idx"].astype(np.int64)
                    # float64(int16) times float64(mult), per element: the
                    # product float(dx_q) * mult was, bit for bit.
                    delta = rec["q"].astype(np.float64) * mult
                else:
                    idx = np.zeros(0, dtype=np.int64)
                    delta = np.zeros((0, 3), dtype=np.float64)
                p += noff * 8
                morphs.append(TriMorph(name=mname, idx=idx, delta=delta))
    ...
    def save(self, path: str | Path) -> None:
        ...
        for sh in self.shapes:
            ...
            for m in sh.morphs:
                ...
                if n > 0xFFFF:
                    raise ValueError(
                        f"morph {m.name!r} has {n} offsets "
                ...
                idxs = m.idx
                if (idxs > 0xFFFF).any() or (idxs < 0).any():
                    raise ValueError(
                ...
                buf = np.empty((n, 4), dtype=np.uint16)
