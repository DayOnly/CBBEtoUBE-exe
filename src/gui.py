...
# The two machine markers the batch prints for the window. "[progress]" opens a
# mod; "[progress-nif]" is printed inside a multi-file mod as files finish, at
# the cadence of the human "[done/total] N NIF/s" line. Both are stripped from
# the visible log. Module-level so the parsers are unit-testable. #per-file-progress
_PROG_RX = re.compile(r"\[progress\] (\d+) (\d+) ([^\n]*)\n?")
_NIF_RX = re.compile(r"\[progress-nif\] (\d+) (\d+)\n?")


def _bar_value(mod_index: int, nif_done: int, nif_total: int) -> float:
    """Where the determinate bar sits, in mods: the mods already finished plus
    this mod's finished share of its files. The marker for mod i means i-1
    mods are done, so the bar no longer jumps a whole mod ahead when one
    starts and then sits still for the whole of a 76-file mod."""
    base = max(0, mod_index - 1)
    if nif_total <= 0:
        return float(base)
    return base + min(max(nif_done, 0), nif_total) / nif_total


def _mod_eta(started: float, nif_done: int, nif_total: int, now: float) -> str:
    """Time left for THIS mod from its own rate so far; 'estimating…' before
    the first file lands. The whole-run estimate stays with the per-mod EWMA:
    the run's file total is not known up front (work items are built per mod
    at its turn), so weighting it by files needs a pre-count, which is the
    cross-mod queue's job when it lands."""
    if nif_done <= 0 or nif_total <= 0 or now <= started:
        return "estimating\u2026"
    per_file = (now - started) / nif_done
...
def launch_gui(argv=None, auto_close_ms=None, _smoke_settings=False) -> int:
    ...
    state = {"running": False, "result": None, "output_dir": None,
             ...
             "cancelled": False}    # set by Cancel, read by _finish  #cancel-says-so
    ...
    # The Releases page, to compare with the build row in Check setup. #releases-link
    help_menu.add_command(
        label="Check for a newer version (web)",
        command=lambda: _open_url(_rt.RELEASES_URL))
    ...
    def _cancel_run():
        ...
        if proc is None:
            return
        state["cancelled"] = True   # _finish words the end from this  #cancel-says-so
    ...
    def _export_diagnostics():
        """Zip the log panel, settings, exclusions, a layout snapshot and a fresh
        preflight, plus what src/diagnostics.py collects from disk (both run
        logs, both failures files, the output mod's report and settings, and
        machine.txt), into one file for sharing when asking for help."""
        import json
        import time as _t
        import zipfile
        ...
        def work():
            ...
            try:
                with zipfile.ZipFile(zpath, "w", zipfile.ZIP_DEFLATED) as z:
                    ...
                    # after a restart following a dead run it carried no log at
                    # all. #diagnostics-zip
                    try:
                        for name, data in diagnostics.collect(
                                _paths.tool_dir(), diag_output_dir).items():
    ...
    def _render_results(data=None):
        ...
        # A run that died mid-batch leaves its checkpoint, marked incomplete,
        # where the previous run's finished report used to read as current;
        # say so in the heading and in the first row. This function owns the
        # heading: the callers that used to reset it to a plain "Last run"
        # threw the date away. #report-checkpoint
        _unfinished = rep.get("complete") is False
        _head = "INCOMPLETE run" if _unfinished else "Last run"
        try:
            import time as _time
            _when = _time.strftime("%Y-%m-%d %H:%M",
                                   _time.localtime(rj.stat().st_mtime))
            _res_head.configure(text=f"{_head} — {_when}")
        except Exception:
        ...
        rows = [
            ...
            # start. A report from an older build has no such key. #orphan-temps
            ("Orphaned temp files removed", rep.get("orphan_temps_removed", 0),
             "warn" if rep.get("orphan_temps_removed") else "ok"),
        ]
    ...
    def _read_run_failures():
        """The failures file this run's child wrote; [] when it is absent or
        unreadable."""
        import json as _json
        ...
        try:
            ...
            return data.get("failures") or []
        except Exception:
            return []

    def _show_failures_popup(fails):
        """End-of-run dialog listing what did not convert and what needs
        attention, grouped by source mod and worded from each entry's severity
        (src/failure_summary.py). An empty list = clean run = no popup."""
        if not fails:
            return
        top = tk.Toplevel(root)
        _theme_popup(top)
        top.title(failure_summary.popup_title(fails))
        top.geometry("760x440")
        ttk.Label(
            top,
            text=failure_summary.popup_intro(fails),
        ...
        txt.insert("1.0", "\n".join(failure_summary.popup_lines(fails)))
    ...
    def _finish(rc):
        ...
        fails = _read_run_failures()
        # "Done - success" over a run with warnings is how a missing SkyPatcher
        # read as a clean finish (#run-warnings); "Finished with exit code 1"
        # over a run the user had just cancelled read as an error, and the
        # popup it opened listed what a killed child never finished writing.
        # The Results tab is still repainted above: the checkpoint report
        # (#report-checkpoint) says what did finish. #cancel-says-so
        cancelled = bool(state.get("cancelled"))
        status.set(failure_summary.status_line(rc, fails, cancelled=bool(state.get("cancelled"))))
    ...
    def _update_progress(done, total, name):
        if total <= 0:
            return
        now = time.time()
        # The marker for mod `done` means done-1 mods are finished; this mod's
        # files fill the bar from there (#per-file-progress).
        state["_mod"] = {"i": done, "n": total, "name": name, "t0": now}
        try:
            if str(prog.cget("mode")) != "determinate":
                prog.stop()
                prog.configure(mode="determinate", maximum=total)
            prog.configure(value=_bar_value(done, 0, 1))
        except Exception:
            pass
        eta = _eta_step(state["_eta"], done, total, now)
        state["_eta_text"] = eta
        status.set(f"Converting {done}/{total}: {name}"
                   + (f" — {eta}" if eta else ""))

    def _update_nif_progress(nif_done, nif_total):
        m = state.get("_mod")
        if not m or nif_total <= 0:
            return
        try:
                ...
                if isinstance(item, tuple) and item and item[0] is _DONE:
                    ...
                    for m in _PROG_RX.finditer(item):
                        _update_progress(int(m.group(1)), int(m.group(2)),
                                         m.group(3).strip())
                    for m in _NIF_RX.finditer(item):
                        _update_nif_progress(int(m.group(1)), int(m.group(2)))
                    text = _NIF_RX.sub("", _PROG_RX.sub("", item))
                    if text:
                        _append(text)
                else:
                    _append(item)
