...
In the window, the bar fills mod by mod and, inside a large mod, file by file,
with a time estimate for the mod it is on beside the estimate for the run.
Cancel stops the run and the status line says so; the Results tab then shows
...
CBBEtoUBE Auto/
  ...
  conversion_summary.txt              the run's coverage summary, written at the end
```
...
| A **single vertex** puckers or spikes on an otherwise smooth panel | A vertex holds four bones at most; making cloth follow the body fills nearly every vertex to that limit, and a near-tie for the fourth place can leave one vertex bending on a different part of the spine than the surface around it | Fixed by **Keep weighting as smooth as the author made it** (§7), on by default. If you still see one, say which panel, and whether it shows standing still or only in motion. |
