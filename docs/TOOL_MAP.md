...
**67** tracked measurement tools. **Check here before hand-rolling a probe.**
...
| `scripts/analysis/fit_audit.py` | 1 | — | Pre-flight fit / clipping auditor for converted UBE armor |
| `scripts/analysis/inflate_census.py` | 2, 3 | yes | PACK CENSUS: does `inflate_armor_outward` still earn its place? |
| `scripts/analysis/morph_clip_test.py` | 3 | — | Morph the BODY and the GARMENT together, then measure the clip |
...
| `scripts/analysis/scan_morph_issues.py` | 1 | — | Programmatic scan of converted NIFs for morph issues across the whole output mod |
...
| `scripts/analysis/verify_weight_invariant.py` | 1 | — | Skinning invariant over the converted output (#weight-write-invariant, P6) |
| `scripts/analysis/verify_zero_weight_bones.py` | 1 | — | Find ZERO-WEIGHT BONES in the converted output (#zeroweight-bone-desync) |
...
| `scripts/analysis/bust_verdict.py` | 3 | — | Decide WHY a piece still shows breast-through-armour in game, when its bust FOLLOW is already at the in-game-validated level |
...
| `scripts/analysis/fold_census.py` | 2 | yes | DOES THE SURFACE PASS THROUGH ITSELF? -- the defect class every position-fidelity metric in this repo is blind to |
| `scripts/analysis/follow_bands.py` | — | — | FOLLOW ratio per anatomical band. |
...
| `scripts/analysis/physics_cloth_health.py` | 2 | — | Physics-cloth health across the pack: WHY a simulated garment clips the body |
