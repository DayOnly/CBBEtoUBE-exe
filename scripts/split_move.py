...
Reads: src/nif_convert.py (LF or CRLF, kept as found), scripts/pass_map.py.
Writes: src/<module>.py (LF), src/nif_convert.py (its own endings, spans replaced by a
...
# The file's own line ending, kept on write. This asserted CRLF, and the file
# has been LF since 2026-09-02, so every run stopped here. #lf-only
NL = "\r\n" if "\r\n" in raw else "\n"
L = raw.split(NL); tree = ast.parse(raw)
...
inner = inner + ("" if inner.endswith(",") else ",") + NL + f'                  "{a.module}",'
