...
os.environ.setdefault("CBBE2UBE_MO2_INI", r"<MODLIST_ROOT>\ModOrganizer.ini")
sys.path.insert(0, r"<USER_HOME>\Downloads\cbbe-to-ube")
...
OUTPUT = Path(r"<MODLIST_ROOT>\mods\CBBEtoUBE Auto")
...
src_dir = Path(r"<MODLIST_ROOT>\mods\RB's Sand Snake Armor and Weapon")
md = _discover_master_data_dirs(src_dir)
print(f"master data dirs: {len(md)} (first: {md[0] if md else None})")

target_esp = LIVE_ESP if COMMIT else Path(r"<USER_HOME>\Downloads\cbbe-to-ube\_tmp_mnb") / "UBE_ModNonBody_Coverage.esp"
